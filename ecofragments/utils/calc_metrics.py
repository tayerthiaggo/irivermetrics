...
# Internal helpers (migrated from wd_batch)
...
def validate(da_wmask, rcor_extent, outdir, section_length, img_ext, section_name_col):
    ...
    if isinstance(da_wmask, str) and os.path.isdir(da_wmask):
        if outdir == None and rcor_extent == None:
            outdir = os.path.join(da_wmask, 'results_ecofragments')
        # Create DataArray and validade rasters within the provided folder
        da_wmask, n_bands, crs = validate_input_folder(da_wmask, img_ext)
...
@delayed
def preprocess_feature(da_wmask, feature, section_name_col, pixel_size, min_patch_size):
    """
    ...
    labeled_layer, labeled_skel, distance_transform_layer = pre_process_layer(
        da_wmask_feature, min_patch_size
    )
...
def export_shapefiles(preprocessed, 
    ...
    polygont_gdf = export_polygons(da_wmask_feature, outdir, section, min_patch_size, pixel_size)
...
def process_metrics(group):
    ...
    if areas.sum() == 0:            
        ...
        PF = n_patches / total_wetted_area
        PLF = np.nan if total_wetted_length == 0 else n_patches / total_wetted_length
...
def validate_shp_input(rcor_extent, img_crs, section_name_col):
    ...
    rcor_extent = validate_input_projection(rcor_extent, img_crs)
...
def setup_directories_cm(rcor_extent, outdir):
    ...
    directory is provided, it defaults to a 'results_ecofragments' folder in the same directory as the rcor_extent file.
    ...
    if outdir == None:
        ...
        outdir = os.path.join(os.path.dirname(os.path.abspath(rcor_extent)), 'results_ecofragments')
    else:
        outdir = os.path.join(outdir, 'results_ecofragments')
...
def find_connected_components(block, min_patch_size):
    structure = np.ones((3, 3), dtype=int) 
    labeled_array, _ = label(block == 1, structure=structure)
    # Remove small objects
    labeled_array = remove_small_objects(labeled_array, min_size=min_patch_size)
...
def pre_process_layer(da_wmask_feature, min_patch_size):
    ...
    labeled_layer = xr.apply_ufunc(
        ...
        kwargs={'min_patch_size': min_patch_size}
    )
...
def export_polygons(da_wmask_feature, outdir, section, min_patch_size, pixel_size):
    """
    ...
    - min_patch_size (int): Minimum number of pixels for a polygon.
    ...
    polygon_gdf = extract_polygons_map_blocks(da_wmask_feature, date_list, transform, crs, section, min_patch_size)
    
    filtered_polygon_gdf = filter_polygons(polygon_gdf, min_patch_size, pixel_size)
    
    return filtered_polygon_gdf
    
    filtered_polygon_gdf.to_file(os.path.join(outdir, f'Polygons_section_{section}.shp'))
def process_polygons_gdf(block, date, transform, crs, section, min_patch_size):
    """
    ...
    mask_filtered = remove_small_objects(mask, min_size=min_patch_size)
...
def extract_polygons_map_blocks(da_wmask_feature, date_list, transform, crs, section, min_patch_size):
    """
    ...
    for i in range(da_wmask_feature.shape[0]):
        ...
        task = delayed(process_polygons_gdf)(block, date, transform, crs, section, min_patch_size)
...
def filter_polygons(polygon_gdf, min_patch_size, pixel_size):
    
    # Calculate minimum area based on min_patch_size and pixel_size
    min_area = min_patch_size * (pixel_size ** 2)  # e.g., 2 * 30^2 = 1800 m²
