...
| `reconnection_timing` | kernel/docs covered; not profile-wired in current `analyze()` | month | Lag after end-dry until connectivity crosses a threshold, preferring RC, then LPSEC, then LPI proxy. Proxy use is explicitly flagged. |
| `refuge_spatial_stability` | kernel/docs covered; not profile-wired in current `analyze()` | dimensionless | Jaccard overlap of consecutive end-dry refuge footprints. First year has no comparison; empty-over-empty is undefined. |
