...
4. **Minimum mapping unit is mandatory.** Default `min_patch_pixels = 3` for Landsat/WOfS-class 30 m products, configurable by source. Patches below the threshold are removed or flagged before patch metrics are calculated. Output must record `min_patch_pixels` and `min_patch_area_m2`.
   - **`[AUDIT FIX 2026-07-19]` Provenance note.** The shipped code default is `min_patch_pixels = 3` (`hydrofragments/config.py`, `PatchesConfig.min_patch_pixels`, and `analyze_patch_bundle`/`analyze_patch_metrics` in `hydrofragments/metrics/patches.py`). This differs from Paper 2's MMU of 2 pixels — report the actually-configured `min_patch_pixels` (and `resolution_m`) alongside every derived `N`/`LPI`/`MESH`/patch-shape claim; never assume 2 or infer it from a prior paper. Likewise, the shipped Refuge Area threshold default is `θ = 0.90` (`persistence.refuge_threshold`, `hydrofragments/config.py`), consumed in `compute_refuge_area` (`hydrofragments/metrics/persistence.py`) as `threshold_pct = refuge_threshold × 100`. Report the configured `θ` alongside every `RA_θ` claim. Both values are recorded in run metadata/config hash per item 7 below, so "report alongside every derived claim" means citing the value actually used for that run, not the spec default, if a run overrides it.
   - **Cross-resolution non-comparability.** `N`, `LPI`, and `MESH` are computed on labelled connected components of a pixel grid and are therefore **resolution-dependent, not just MMU-dependent**: comparing `N`/`LPI`/`MESH` between (for example) a 30 m WOfS-class run and a 10 m Sentinel-2 run is not a like-for-like ecological comparison even if `min_patch_pixels` is nominally "the same" in pixel units, because the same pixel count spans a different real area at each resolution. Always report `resolution_m` and `min_patch_pixels` together and restrict direct numeric comparison to same-resolution, same-MMU runs.
...
|---|---|---|---|---|
| Occurrence frequency (per pixel) | **Keep (replaces Pixel Persistence)** | Verbatim (Pekel et al. 2016, *Nature*; Mueller et al. 2016, DEA WOfS) | water obs / valid obs, per pixel (season-stratified equal-weight-over-months as shipped — see §6.17 `[AUDIT FIX 2026-07-19]`) | Standard, comparable, replaces bespoke PP. **`[AUDIT FIX]`** explicitly cite Pekel/Mueller — this is a JRC-GSW/DEA-WOfS-style layer, not a novel construction. |
...
- **`[AUDIT FIX 2026-07-19]` NaN behaviour is all-or-nothing per run, not per pool.** As shipped (`compute_patch_metrics` in `hydrofragments/metrics/patches.py`), `AWRe` is computed only if **every** patch in the run has a finite, positive length (`np.all(np.isfinite(lengths) & (lengths > 0))`); if even one patch's length is non-finite or non-positive (e.g. a degenerate single-pixel or zero-length skeleton segment), the **entire** AOI/window `AWRe` value for that month is `NaN`, not a partial mean excluding the bad pool. Document this explicitly: a single malformed patch can silently null out an otherwise-valid AWRe series entry, so QA should check for and report unexpected `NaN` runs of `AWRe` rather than assuming missingness is always due to `N=0`.
...
- **Definition — recurrence (conceptual, v1.1):** for each pixel, `REC_p = count(years where pixel is wet at least once) / count(valid years) × 100`. **`[AUDIT FIX 2026-07-19]`** this "years wet at least once" formulation is the *original conceptual* definition and is **not** what ships. The reference implementation (`compute_recurrence`, `hydrofragments/metrics/persistence.py`) instead computes the season-stratified equal-weight-over-calendar-months construction described below and in §6.17 — cite that as `REC_p`'s actual formula; `valid_year_count` is retained only as a support diagnostic, not as the denominator of the reported value.
- **Definition — annual seasonality / hydroperiod:** for each pixel and year, `SEAS_{p,y} = count(months wet in year y)` or `HP_{p,y} = valid wet months / valid observed months`. This matches the shipped `compute_hydroperiod`; note it is a **fraction in [0, 1]**, not a percentage (see unit-convention note below).
...
- **`[AUDIT FIX 2026-07-19]` Unit convention — do not mix percent and fraction.** As shipped, **hydroperiod is a fraction in [0, 1]**, not a percentage: `compute_hydroperiod` (`hydrofragments/metrics/persistence.py`) returns `HP_{p,y} = wet_months / valid_observed_months` with no `× 100`. **Occurrence and recurrence are percentages in [0, 100]**: both `compute_occurrence` and `compute_recurrence` apply `× 100.0` to their respective per-pixel ratios. Any table, plot, or paper text mixing these three must convert explicitly (`HP × 100` to compare against `OCC`/`REC` on the same percent scale) rather than assume a shared unit; the code does not normalise this automatically and there is no guard against silently comparing a 0–1 value to a 0–100 one.
- **`[AUDIT FIX 2026-07-19]` Recurrence is structurally the occurrence estimator's core, applied per year rather than per pixel-record.** `compute_recurrence` (`hydrofragments/metrics/persistence.py`) uses the *same* season-stratified equal-weight-over-calendar-months construction as `compute_occurrence` (§6.17) — group wet-and-valid by calendar month, ratio to valid count per calendar month, then unweighted mean across supported months × 100 — so `REC_p` and `OCC_p` are computed by structurally identical logic over the same monthly water/valid-obs inputs, and will be numerically close (not identical, since `compute_recurrence` additionally tracks `valid_year_count` as a support diagnostic and the two are conceptually distinct: occurrence is a long-term wet-frequency estimator, recurrence is framed as inter-annual reliability). Do not treat them as independent lines of evidence when both are reported for the same pixel; they share an estimator core.
...
- **`OCC_p`** — **`[AUDIT FIX 2026-07-19]`** the pooled ratio `Σ_t W_{p,t} / Σ_t V_{p,t} × 100` shown in v1.1/early v1.2 drafts is **not** what ships. Decision Gate 0 (U2/Q1, approved 2026-07-14) supersedes it with a **season-stratified equal-weight mean**, because the pooled ratio is biased by confirmed seasonal missing-not-at-random (MNAR) observation gaps (`docs/audit/evidence/validity_reliability_report.md` §4): a calendar month that happens to have more valid observations in the record would otherwise be over-weighted relative to a sparsely-observed month. The shipped estimator, exactly as implemented in `hydrofragments/metrics/persistence.py::compute_occurrence` / `_season_stratified_occurrence`:
  ...
  1. For each calendar month `m ∈ {1..12}` present in the record, compute the pooled-over-years ratio for that calendar month only: `ratio_m = Σ_{t: month(t)=m} (W_{p,t} ∧ V_{p,t}) / Σ_{t: month(t)=m} V_{p,t}`. A calendar month with zero valid observations across all years contributes no term (its ratio is `NaN` and is dropped, not treated as 0).
  2. `OCC_p = mean_m(ratio_m, skipna=True) × 100` — the unweighted (equal-weight) mean of the supported calendar-month ratios, as a percentage.
  3. Denominators throughout are **valid-observation counts**, never raw timestep counts; unobserved months enter neither numerator nor denominator (Decision Gate 0).
  4. `OCC_p` is suppressed to `NaN` where `valid_count_p < min_valid_obs` (spec §8 guard 4), independent of the stratified computation above.

  **Cite the stratified form as the shipped occurrence estimator** in any paper text or downstream documentation. The pooled formula (`Σ_t W/Σ_t V × 100`) remains a documented *alternative* — it is simpler to explain and may be preferred for a paper that argues seasonal MNAR is not material for a given catchment — but it is not what the reference implementation computes by default, and results computed from it are not directly comparable to `OCC_p` as shipped without saying so explicitly.
  - **Recurrence** (`REC_p`, §6.12) uses the same season-stratified equal-weight-over-calendar-months construction (`compute_recurrence`); see the note there.
- `RA_θ = cell_area × count(OCC_p ≥ θ and valid_count_p ≥ min_valid_obs)`. Shipped default `θ = 0.90` (90%, `persistence.refuge_threshold` in `hydrofragments/config.py`) and `min_valid_obs = 20` (`validity.min_valid_obs`); see §1.1 provenance note below — always report the actual configured values alongside any `RA_θ` claim, not the defaults from memory.
...
**`[AUDIT FIX 2026-07-19]` Provenance and cross-resolution comparability.** Every reported `N`, `LPI`, or `MESH` value must be accompanied by the `min_patch_pixels` (MMU) value and source `resolution_m` used to produce it (both are already mandatory output columns, §7.1). `N`, `LPI`, and `MESH` are **not comparable across runs at different pixel resolutions** (e.g. a 30 m Landsat/WOfS-class run vs a 10 m Sentinel-2 run): the same physical waterbody population yields a different patch count, dominance fraction, and fragmentation index purely from the change in minimum resolvable patch size and boundary pixelation, independent of any real ecological difference. Do not rank or compare `N`/`LPI`/`MESH` across sources or resolutions without first normalising for MMU/resolution (or restricting comparison to a single fixed source/resolution).
