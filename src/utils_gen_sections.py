...
from shapely.geometry import MultiPolygon, GeometryCollection, Polygon, MultiPoint, MultiLineString
from rtree import index
from shapely.ops import snap, split, linemerge

from utils_calc_metrics import validate, match_input_extent, fill_nodata, calculate_pixel_persistence

def validate_and_preprocess(da_wmask, r_lines, str_order_col):
    """
    ...
    da_wmask, r_lines = validate(da_wmask, r_lines, section_length=None, img_ext='.tif', expected_geometry_type='LineString')
    # Preprocess the inputs `da_wmask` and `r_lines` to make them ready for further processing
    da_wmask, r_lines = preprocess_inputs(da_wmask, r_lines)
...
def preprocess_inputs(da_wmask, rcor_extent):
    """
